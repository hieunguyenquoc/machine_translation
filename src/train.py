...
for epoch in range(1, NUM_EPOCHS+1):
    start_time = timer()
    train_loss = train_epoch(hyper_parameter.transformer, hyper_parameter.optimizer)
    end_time = timer()
    val_loss = evaluate(hyper_parameter.transformer)
...
def greedy_decode(model, src, src_mask, max_len, start_symbol):
    ...
    for i in range(max_len-1):
        ...
        if next_word == Data_preprocess.EOS_IDX:
            break
    return ys
